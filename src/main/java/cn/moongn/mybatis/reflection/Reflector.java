...
import cn.moongn.mybatis.reflection.invoker.GetFieldInvoker;
import cn.moongn.mybatis.reflection.invoker.Invoker;
import cn.moongn.mybatis.reflection.invoker.MethodInvoker;
import cn.moongn.mybatis.reflection.invoker.SetFieldInvoker;
import cn.moongn.mybatis.reflection.property.PropertyNamer;
...
public class Reflector {
            ...
            if (getters.size() == 1) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (methodType.equals(getterType)) {
                        ...
                        // getter type is descendant
                    } else if (methodType.isAssignableFrom(methodType)) {
                        getter = method;
    ...
    public Class<?> getSetterType(String propName) {
        Class<?> clazz = getTypes.get(propName);
        if (clazz == null) {
    ...
    public Class<?> getGetterType(String propName) {
        Class<?> clazz = getTypes.get(propName);
        if (clazz == null) {
            throw new RuntimeException("There is no getter for property named '" + propName + "' in '" + type + "'");
        }
        return clazz;
    }
    ...
    private void addGetField(Field field) {
        if(isValidPropertyName(field.getName())) {
            setMethods.put(field.getName(), new GetFieldInvoker(field));
            setTypes.put(field.getName(), field.getType());
        }
    }

    public Invoker getGetInvoker(String propName) {
        Invoker method = getMethods.get(propName);
        if (method == null) {
            throw new RuntimeException("There is no getter for property named '" + propName + "' in '" + type + "'");
        }
        return method;
    ...
    public Invoker getSetInvoker(String propName) {
        Invoker method = setMethods.get(propName);
        if (method == null) {
            throw new RuntimeException("There is no setter for property named '" + propName + "' in '" + type + "'");
        }
        return method;
